...
7. **配置不得成为权限的来源**：主体、角色、权限码、作用域一律不得由配置文件或环境变量提供，默认作用域只能来自主体的绑定关系。新增一个配置项前先问一句"这个键能让谁获得什么"，答案不是"什么也不能"就不该存在。同理，任何以配置项形式出现的"跳过鉴权""信任该来源"开关都不允许存在——这类开关一旦存在就一定会被用在生产环境。
   ...
   **唯一例外是引导**（`bootstrap_admin_subject` / `bootstrap_admin_scope`）：系统里还没有任何角色绑定时，用它建立第一个管理员。区分"引导"与"提权"的判据是**配置是初始化的输入，不是判定的输入**。它必须同时满足四条，缺一即不是引导：
   ...
   - **按不可复用的标识**：取值必须是主体标识（由 aladdin 分配、分配即冻结、永不复用），不得用邮箱，也不得用渠道上的身份标识。邮箱在渠道侧可改名、可回收，回收给另一个人的那天就是一次无痕提权。

违反 1–3 中的任何一条，构建期的 `internal/rbac/catalog_test.go` 会失败；第 7 条引导例外的四条边界由 `internal/server` 的测试固定（存储非空时不生效、两个键只填一个时拒绝启动）。
