...
7. **配置不得成为权限的来源**：主体、角色、权限码、作用域一律不得由配置文件或环境变量提供，默认作用域只能来自主体的绑定关系。新增一个配置项前先问一句"这个键能让谁获得什么"，答案不是"什么也不能"就不该存在。同理，任何以配置项形式出现的"跳过鉴权""信任该来源"开关都不允许存在——这类开关一旦存在就一定会被用在生产环境。
   ...
   **唯一例外是引导**（`bootstrap_admin_subject` / `bootstrap_admin_email` / `bootstrap_admin_scope`）：系统里还没有任何角色绑定时，用它建立第一个管理员。区分"引导"与"提权"的判据是**配置是初始化的输入，不是判定的输入**。它必须同时满足四条，缺一即不是引导：

   - **物化**：写入的是一条真实的角色绑定；判定路径只读存储、从不读配置。改这个键不改变任何一次判定结果。
   - **一次性**：只在存储中不存在任何绑定时生效。生效后键即可删除；删掉它**不**撤销已建立的绑定。
   - **留痕**：生效时以 `warn` 级别记录被授予的主体与作用域，与开发种子旁路同级。
   - **落点必须是不可复用的主体标识**：绑定写在主体标识上（由 aladdin 分配、分配即冻结、永不复用）。**任何一次判定都不得按邮箱或渠道身份标识确定身份**——邮箱在渠道侧可改名、可回收，回收给另一个人的那天就是一次无痕提权。

     `bootstrap_admin_email` 是这条规则的一处**窄口子，且仅限引导**：它在生效的那一刻按邮箱查出**已登记**的身份、取用其主体，此后不再参与任何判定。本库里同一个邮箱字符串可以分别挂在两个身份上（这是刻意允许的），所以**命中数不是恰好一个就拒绝启动**——不猜。它不构成"身份可以按邮箱确定"的先例：登录与绑定两条路径仍只认渠道签发的不可变标识。

违反 1–3 中的任何一条，构建期的 `internal/rbac/catalog_test.go` 会失败；第 7 条引导例外的四条边界由 `internal/server` 的测试固定（存储非空时不生效、身份指认不唯一时拒绝启动、只填一半时拒绝启动）。
